...
def get_current_worker():
    ...
    return Worker.query.filter_by(username=username).first() if username else None
...
def update_status(lead_id):
    ...
    if action == "no":
        ...
        if not send_make_webhook(lead, customer_email, worker.username):
            return jsonify({"ok": False, "error": "Webhook failed"}), 502
        lead.status         = "closed"
...
def create_worker():
    if request.headers.get("X-API-Key") != os.environ.get("ADMIN_API_KEY", "secret-key"):
        return jsonify({"ok": False, "error": "Unauthorized"}), 401
    data = request.get_json()
    worker = Worker(username=data["username"], password=data["password"])
